...
void gb_interrupt_controller::add_interrupt_source(gb_interrupt_source* interrupt_source) {
    ...
    std::sort(m_interrupt_sources.begin(), m_interrupt_sources.end(),
        [](gb_interrupt_source* a, gb_interrupt_source* b) -> bool {
...
void gb_interrupt_controller::update(int cycles) {
    ...
    for (gb_interrupt_source* isource : m_interrupt_sources) {
        ...
        if (interrupt_raised) {
            ...
            highest_priority_isource = isource;
        }
    }
    ...
    if (highest_priority_isource != nullptr) {
        ...
        if ((m_ienable.read_byte(GB_IENABLE_ADDR) & flag_mask) && m_cpu.handle_interrupt(highest_priority_isource)) {

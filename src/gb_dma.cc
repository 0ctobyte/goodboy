...
gb_dma::gb_dma(gb_memory_manager& memory_manager, gb_memory_map& memory_map)
    : gb_memory_mapped_device(memory_manager, GB_DMA_ADDR, 1),
      m_memory_map(memory_map), m_bytes_transferred(160), m_delay_cycles(0)
{
}
...
bool gb_dma::update(int cycles) {
    ...
    // Make sure the bottom four bits of every fourth byte are zeros
    ...
    for (int i = n_cycles; i > 0 && m_bytes_transferred < 160; i -= 4, m_bytes_transferred++) {
        ...
        src_byte = ((m_bytes_transferred & 0x3) == 3) ? src_byte & 0xf0 : src_byte;
        m_memory_map.write_byte(static_cast<uint16_t>(GB_PPU_OAM_ADDR + m_bytes_transferred), src_byte);
    }
